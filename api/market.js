// /api/markets — список топ-криптовалют с ценами через CoinGecko (ключ скрыт в env).
// GET /api/markets           -> топ ~120 монет (цена, изменение 24ч, лого, капа)
// GET /api/markets?page=2    -> следующая страница
// Требует env: COINGECKO_API_KEY (Demo key вида CG-...). Работает и без ключа (публичный лимит), но с ключом стабильнее.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Отдаём компактный формат, понятный фронтенду
    ...
    // кэш на CDN Vercel: свежесть 30с, чтобы не бить лимиты
